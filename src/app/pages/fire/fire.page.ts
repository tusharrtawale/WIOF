...
export class FirePage implements OnInit {
  ...
  playlistIdForFirePage:String="PLitAAO2FhPTFw5EoLsuua6Ang60PGBH0d";
  slideOpts = {
    slidesPerView: 5,
    freeMode: true,
    coverflowEffect: {
  ...
  ngOnInit() {
    this.blogs = this.blogService.getBlogs("Fire");

    this.vids_frontend=this.videoService.getYoutubePlaylist(this.playlistIdForFirePage).pipe(map(data=>{
