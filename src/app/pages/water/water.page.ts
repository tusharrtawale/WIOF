...
export class WaterPage implements OnInit {
  ...
  playlistIdForWaterPage:String="PLitAAO2FhPTHZRcarOENNjYcdPlzwpZEn";
  slideOpts = {
    slidesPerView: 5,
    freeMode: true,
    coverflowEffect: {
  ...
  ngOnInit() {
    this.blogs = this.blogService.getBlogs("Water");

    this.vids_frontend=this.videoService.getYoutubePlaylist(this.playlistIdForWaterPage).pipe(map(data=>{
